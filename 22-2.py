...
for i, brick in enumerate(bricks):
    current_height, _ = max(
        (grid[(x, y)] for x in brick.x for y in brick.y), key=lambda p: p[0]
    )
...
def chain_reaction(b, destroyed):
    brick = bricks[b]
    if all(i in destroyed for i in brick.supported_by):
        destroyed.add(b)
    for i in brick.supports:
        chain_reaction(i, destroyed)


total = 0
for b in range(len(bricks)):
    destroyed = {b}
    chain_reaction(b, destroyed)
    print(b, destroyed)
